...
async function handleSubmit(event) {
    ...
    try {
        ...
        showMessage(
            "success",
            `Đăng ký thành công tài khoản ${response.email}.`
        );
...
function validate(payload) {
    ...
    if (!payload.lastName) {
        ...
        errors.lastName =
            "Họ và tên đệm không được vượt quá 150 ký tự.";
    }
    ...
    if (!payload.password) {
        ...
        errors.password =
            "Mật khẩu phải có từ 8 đến 64 ký tự.";
    }
...
function setSubmitting(isSubmitting) {
    ...
    registerButton.textContent = isSubmitting
        ? "Đang đăng ký..."
        : "Đăng ký";
}
