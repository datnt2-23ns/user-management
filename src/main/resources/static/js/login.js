...
import { isAuthenticated, saveAuthSession } from "/js/auth.js";
...
async function handleSubmit(event) {
  ...
  if (Object.keys(validationErrors).length > 0) {
    ...
    showMessage("error", "Vui lòng kiểm tra lại email và mật khẩu.");
  ...
  try {
    ...
    showMessage(
      ...
      `Đăng nhập thành công. Xin chào ${
        response.user?.fullName || response.user?.email || "bạn"
      }.`,
    );
...
function handleLoginError(error) {
  ...
  if (error.status === 403) {
    showMessage(
      "error",
      error.data?.message || "Tài khoản đã bị khóa hoặc không còn hoạt động.",
    );
...
function togglePasswordVisibility() {
  ...
  togglePasswordButton.setAttribute(
    "aria-label",
    isHidden ? "Ẩn mật khẩu" : "Hiển thị mật khẩu",
  );
